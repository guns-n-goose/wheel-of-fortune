<script>
  ...
  const winEvent = () => {dispatch('winEvent', {amount: 50})};
  const loseEvent = () => {dispatch('loseEvent'), {amount: 50}};
  let deg = 0;
  let zoneSize = 45; // deg
  // Counter clockwise
  ...
  const handleWin = (actualDeg) => {
    ...
    if (display.innerHTML === symbolSegments[winningSymbolNr]) {
      ...
      dispatch('winEvent', {amount: 50})
    }
    else {
      display.innerHTML = '-50';
      dispatch('loseEvent', {amount: 50})
    }
...
<main transition>
  <div id="app">
    ...
    <div class="inline-flex rounded-md shadow-sm" role="group">
      <button
        type="button"
        class="{rolling ? 'cursor-not-allowed' : ''} py-2 px-4 text-sm font-medium text-gray-900 bg-white rounded-l-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
        id="frog"
        disabled = {rolling}
      >
        🐸
      ...
      <button
        type="button"
        class="{rolling ? 'cursor-not-allowed' : ''} py-2 px-4 text-sm font-medium text-gray-900 bg-white border-t border-b border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
        id="snail"
        disabled = {rolling}
      >
        🐌
      ...
      <button
        type="button"
        class="{rolling ? 'cursor-not-allowed' : ''} py-2 px-4 text-sm font-medium text-gray-900 bg-white rounded-r-md border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
        id="dolphin"
        disabled = {rolling}
      >
        🐬
